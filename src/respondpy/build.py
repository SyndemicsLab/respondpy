...
def build_model(
    ...
    *,
    ...
    times = list(map(int, input_data.config.get(
        "simulation", "parameter_change_times").split()))
    model.add_timestep(build_timestep(input_data, cohort_id))
    return model
...
def build_default_transitions(
    ...
    *,
    ...
    d.add_matrix(input_data.select_parameter(
        Parameter(ParameterType.STANDARD_MORTALITY_RATIO), cohort_id, time))
